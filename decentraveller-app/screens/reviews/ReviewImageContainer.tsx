...
const ReviewImageContainer: React.FC<{reviewId: number, placeId: number, imageCount: number}> = ({
    ...
    const renderImage = ({ item }: { item }) => (
        <TouchableOpacity style={placeReviewsBoxStyles.imageContainer} onPress={() => {openGallery(item.id)}}>
            <Image style={placeReviewsBoxStyles.image}
                source={{uri: item.url}}
                   height={100}
                   width={100}
                   resizeMethod={"auto"}
    ...
    if(imageCount > 0) {
        return (
            <TouchableOpacity activeOpacity={1} style={placeReviewsBoxStyles.imagesContainer}>
                    ...
                    <FlatList style={placeReviewsBoxStyles.imageFlatList}
                              data={imagesData} renderItem={renderImage} horizontal={true}
                    scrollEnabled={true} keyExtractor={(item) => (item.url)}/>
                <LinearGradient style={placeReviewsBoxStyles.moreImages}
                                colors={['rgba(240,240,240,1)', 'transparent']}
                                start={{x: 0.8, y: 1.0}} end={{x: 0.0, y: 1.0}}>
